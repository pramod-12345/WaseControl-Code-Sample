...
class Index extends Component {
  ...
  async requestPermissions() {
    await Location.requestForegroundPermissionsAsync().then(async (status) => {
      if (status.status === "granted") {
        await Location.getCurrentPositionAsync({}).then((coord) => {
          ...
          // this.animateToNewRegion(region);
        });
      }
    });
  }
        ...
        {!this.state.isCentered && (
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              // setTimeout(() => {
              //   this.state.isTrack && this.setState({ isTrack: false });
              // }, 1500);
              this.animateToNewRegion(this.state.currentLocation);
            }}
          >
            <Text
              style={[
