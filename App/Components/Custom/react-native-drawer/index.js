...
export default class Drawer extends Component {
    ...
    if (props.initializeOpen || props.open) {
      ...
      if (props.type === 'overlay')
        styles.drawer[this.props.side] =
          this._offsetClosed + this._offsetOpen - fullWidth
      if (props.type === 'displace')
        styles.drawer[this.props.side] =
          -fullWidth + this._offsetClosed + this._offsetOpen
    }
  ...
  updatePosition = () => {
    ...
    let ratio =
      (this._left - this._offsetClosed) /
      (this.getOpenLeft() - this._offsetClosed)
    ...
    switch (this.props.type) {
      case 'overlay':
        ...
        Math.round(-this.state.viewport.width + this._offsetOpen + this._left)
      ...
      case 'displace':
        mainProps[this.props.side] = this._left
        drawerProps[this.props.side] =
          -this.state.viewport.width + this._left + this._offsetOpen
        break
    }
  ...
  shouldOpenDrawer(dx) {
    ...
    if (!hasActiveHeading) return this._open
      ...
      return (
        this._open ^
        (Math.abs(dx) > this.state.viewport.width * this.props.panThreshold)
      )
  }
  ...
  onPanResponderMove = (e, gestureState) => {
    ...
    if (this._open ^ (gestureState.dx < 0) ^ (this.props.side === 'right'))
      return false

    let dx =
      this.props.side === 'right' ? gestureState.dx * -1 : gestureState.dx
  ...
  processMoveShouldSet = (e, gestureState) => {
    ...
    let swipeUpDown =
      Math.abs(gestureState.dy) >= Math.abs(gestureState.dx) ? true : false
    let swipeInCloseDirection =
      this.props.side === 'left' ? swipeToLeft : swipeToRight
    if (
      swipeUpDown ||
  ...
  testPanResponderMask = (e, gestureState) => {
    ...
    let deltaOpen =
      this.props.side === 'left' ? this.state.viewport.width - x0 : x0
    let deltaClose =
      this.props.side === 'left' ? x0 : this.state.viewport.width - x0
  ...
  getOpenMask = () => {
    ...
    if (this.props.panCloseMask)
      return this.state.viewport.width * this.props.panCloseMask
  ...
  getClosedMask = () => {
    ...
    if (this.props.panOpenMask)
      return this.state.viewport.width * this.props.panOpenMask
  ...
  getOpenOffset = (props, viewport) => {
    ...
    return props.openDrawerOffset % 1 === 0
      ...
      : props.openDrawerOffset * viewport.width
  }
  getClosedOffset = (props, viewport) => {
    ...
    return props.closedDrawerOffset % 1 === 0
      ...
      : props.closedDrawerOffset * viewport.width
  }
    ...
    return (
      <Animated.View
        ...
        style={[
          ...
          {
            shadowOpacity: this.state.scale < 1 ? 0.5 : 0,
            backgroundColor: '#ccc',
    ...
    return (
      <View
        ...
        style={[
          ...
          { height: this.getHeight(), width: this.getDrawerWidth() },
        ]}
      >
        {this.props.content}
        <View
